...
export default function GigCard({
  ...
  const buttonConfig = {
    applied: (
      <button
        disabled
        className="w-full mt-4 bg-gray-300 text-white py-2 rounded-md font-medium"
      >
        Applied
      </button>
    ),
    "in-progress": (
      <button
        disabled
        className="w-full mt-4 bg-green-500 text-white py-2 rounded-md font-medium"
      >
        In Progress
      </button>
    ),
    ...
    >
      <div className="relative h-40 overflow-hidden">
        <img
          ...
          onError={() => {
            console.log(gig.image , "--- iamge ---> profile " , gig.user.profilePic);
          }}
        />
      </div>
      <div className="p-4 flex-grow flex flex-col">
        ...
        <div className="mt-auto">{buttonConfig[type]}</div>
      </div>
    </motion.div>
  );
}
