...
import CreatorCampaigns from "./CreatorCampaign";
...
export default function Dashboard() {
  ...
  // Tab states
  const [mainTab, setMainTab] = useState<"Recommended Deals" | "My Deals">(
    "Recommended Deals"
  );
  ...
  const transformCampaignToGig = useCallback((campaign: any): Gig => {
    return {
      ...
      user:{
        profilePic:campaign?.user?.profilePic
      },
      stats: [
  ...
  const fetchRecommendedCampaigns = useCallback(async () => {
    try {
      ...
      setAllGigs((prev) => ({
        ...prev,
        Recommended: Array.isArray(campaigns)
          ? campaigns.map(transformCampaignToGig)
          : [],
      }));
    } catch (err) {
  ...
  // const fetchCampaignsByTab = useCallback(
  //     //@ts-expect-error - wtf
  //   async (tab: keyof omit<GigTabs, "Recommended">) => {
  //     try {
  //       setLoading(true);
  //       setError("");
  //       let campaigns;
  ...
  }, [mainTab, myDealsTab, fetchRecommendedCampaigns, ]);
  ...
  const renderContent = () => {
    ...
    if (mainTab === "Recommended Deals") {
      ...
      return (
        ...
        //         onClick={() =>
        //             //@ts-expect-error - wtf
        //           setMyDealsTab(tab as keyof omit<GigTabs, "Recommended">)
        //         }
        //       >
        //         {tab}
        //       </button>
