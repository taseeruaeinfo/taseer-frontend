...
interface Message {
  ...
  createdAt: string;
  type: string;
  tempId?: string; // Add tempId for tracking
}
...
interface UseSocketReturn {
  ...
  sendMessage: (receiverId: string, content: string) => string; // Return tempId
...
export const useSocket = (token: string | null): UseSocketReturn => {
  ...
  const setupEventListeners = useCallback((socket: Socket) => {
    ...
    socket.on("online_users", (users: OnlineUser[]) => {
      ...
      window.dispatchEvent(
        new CustomEvent("online_users_updated", { detail: users })
      );
    });
    ...
    socket.on("message_sent", (data: { message: Message; tempId?: string }) => {
      if (!mountedRef.current) return;
      console.log("✅ Message sent:", data.message.id, "tempId:", data.tempId);
    ...
    socket.on("conversation_updated", (data: any) => {
      ...
      window.dispatchEvent(
        new CustomEvent("conversation_updated", { detail: data })
      );
    });

    socket.on(
      "unread_count_updated",
      (data: { partnerId: string; unreadCount: number }) => {
        if (!mountedRef.current) return;
        console.log("🔢 Unread count updated:", data);
        window.dispatchEvent(
          new CustomEvent("unread_count_updated", { detail: data })
        );
      }
    );
    ...
    socket.on("message_error", (error: { error: string; tempId?: string }) => {
      if (!mountedRef.current) return;
  ...
  const connect = useCallback(() => {
    ...
    const newSocket = io("https://api.taseer.app/", {
      auth: { token },
  ...
  const joinConversation = useCallback(
    (partnerId: string) => {
      if (socket && connectionStatus === "connected") {
        const roomName = [socket.id?.split("").slice(0, 10).join(""), partnerId]
          .sort()
          .join("_");
